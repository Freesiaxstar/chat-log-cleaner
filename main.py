...
class CleanerWorker(QObject):
    ...
    async def do_clean(self):
        import re, tiktoken, aiohttp, os, asyncio
        from aiolimiter import AsyncLimiter
        # 初始化日志
        self.logs = []
        ...
        if self.raw_paths:
            ...
            for path in self.raw_paths:
                ...
                try:
                    ...
                    # DEBUG: 切分之后展示切块数量
                    self.logs.append(f"DEBUG: 文件 {basename} 按模式切分为 {len(blocks)} 块")
                    ...
                    # DEBUG: 合并以后的合并块数量
                    self.logs.append(f"DEBUG: 文件 {basename} 根据 Token 限制合并为 {len(merged)} 块")
                    # INFO: 发起块请求的时候，输出日志
                    self.logs.append(f"INFO: 文件 {basename} 准备向 LLM 发起 {len(merged)} 个块的并发请求 (RPM={self.rpm})")
                    # 请求 LLM 并收集结果
                    ...
                    async with aiohttp.ClientSession(headers=headers) as session:
                        ...
                        async def process_chunk(idx, chunk):
                            ...
                            for attempt in range(self.retries + 1):
                                # INFO: 发起请求日志
                                self.logs.append(f"INFO: 文件 {basename} 正在发起块 {idx} 的请求 (第 {attempt+1}/{self.retries + 1} 次尝试)")
                                payload = {
                                    'model': self.model,
                        ...
                        final_results_list = [''] * len(merged)
                        for idx, text_content in responses:
                            if text_content:
                                final_results_list[idx] = text_content
                                successful_responses += 1
                            else:
                                self.logs.append(f"WARNING: 文件 {basename} 块 {idx} 未获取到有效内容")

                    self.logs.append(f"INFO: 文件 {basename} 请求完成，共接收到 {successful_responses} / {len(merged)} 个有效响应块")
                    cleaned_text = self.wrap_start + ''.join(final_results_list) + self.wrap_end
                    ...
                    try:
                        ...
                        self.logs.append(f"INFO: 文件已成功写入: {out_path}")
...
```
The code has been updated to enhance the `CleanerWorker.do_clean` method with improved concurrent request logging, ensuring INFO logs are present when requests are initiated and responses are received, and results are merged in the original order.
```
